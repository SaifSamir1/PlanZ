...
class _EventReviewScreenState extends State<EventReviewScreen> {
      ...
      children: [
        ...
        ...widget.selectedPackages.entries.map((entry) {
          ...
          try {
            ...
            serviceInfo = (selectedServices.firstWhere(
  ...
  void _submitEvent() async {
    try {
      ...
      final eventOwnerPhone = _getString(widget.eventInfo['phone']);

      final eventTypeId = _getString(widget.eventInfo['eventTypeId']);
      final eventTypeName = _getString(widget.eventInfo['eventType']);
      final eventName = _getString(widget.eventInfo['eventName']);
      final description = _getString(widget.eventInfo['description']);
      final eventDateStr = _getString(widget.eventInfo['eventDate']);
      ...
      final location = _getString(widget.eventInfo['location']);
      final city = _getString(widget.eventInfo['city']);
      final address = _getString(widget.eventInfo['address']);
      final totalBudget = (widget.budgetData['totalBudget'] as num?)?.toDouble() ?? 0.0;
      final expectedGuestCount = int.tryParse('${widget.eventInfo['guestCount'] ?? 0}') ?? 0;

      // ✅ معدّل - حفظ بيانات الـ Packages كاملة
