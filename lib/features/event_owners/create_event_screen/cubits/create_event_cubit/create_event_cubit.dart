...
class EventOwnerCubit extends Cubit<EventOwnerState> {
  ...
  Future<void> createEvent({
    ...
    debugPrint('🚀 [EventOwnerCubit.createEvent] Starting...');
    debugPrint('   eventName: $eventName');
    debugPrint('   totalBudget: $totalBudget');
    debugPrint('   customRequirements keys: ${customRequirements?.keys.toList()}');
    debugPrint('   selectedPackagesData count: ${(customRequirements?['selectedPackagesData'] as List?)?.length}');
    ...
    result.fold(
      (failure) {
        debugPrint('❌ [EventOwnerCubit.createEvent] Error: ${failure.message}');
        emit(CreateEventError(failure.message));
      },
      (event) {
        debugPrint('✅ [EventOwnerCubit.createEvent] Success! Event ID: ${event.eventId}');
        debugPrint('   Services count: ${event.services.length}');
  ...
  Future<void> getEventOwnerEvents(String eventOwnerId) async {
    debugPrint('');
    debugPrint('🔄 [EventOwnerCubit.getEventOwnerEvents] Starting...');
    ...
    result.fold(
      (failure) {
        debugPrint('❌ [EventOwnerCubit.getEventOwnerEvents] Error: ${failure.message}');
        emit(GetEventOwnerEventsError(failure.message));
      },
      (events) {
        debugPrint('✅ [EventOwnerCubit.getEventOwnerEvents] Success!');
        debugPrint('   Total events: ${events.length}');
        for (var i = 0; i < events.length; i++) {
          debugPrint('   ${i + 1}. ${events[i].eventName} - Paid: ${events[i].paidAmount}');
        }
        emit(GetEventOwnerEventsSuccess(events));
      },
    );
  }
