...
class _EventPackagesScreenState extends State<EventPackagesScreen> {
    ...
    return Scaffold(
      ...
      body: BlocListener<EventOwnerCubit, EventOwnerState>(
        listener: (context, state) {
          // ✅ Listen for ReplacePackageSuccess
          if (state is ReplacePackageSuccess) {
            debugPrint('✅ [ReplacePackageSuccess] Package replaced successfully!');
            debugPrint('   New Event: ${state.event.eventId}');
            debugPrint('   Services: ${state.event.services.length}');
            
            // ✅ إغلاق الـ loading dialog
            Navigator.of(context).pop();
            
            // ✅ Reload data after replacement
            _loadDataAsync();
            
            // ✅ Show success message
            ScaffoldMessenger.of(context).showSnackBar(
              const SnackBar(
                content: Text('✅ Package replaced successfully!'),
                backgroundColor: Colors.green,
                duration: Duration(seconds: 2),
              ),
            );
          } else if (state is ReplacePackageError) {
            debugPrint('❌ [ReplacePackageError] ${state.message}');
        ...
        child: BlocBuilder<EventOwnerCubit, EventOwnerState>(
  ...
  Widget _buildEventServiceCard(
    ...
    final isApproved = service.vendorApproved;
            ...
            child: Column(
              ...
              children: [
                Row(
                  ...
                  children: [
                    Expanded(
                      child: _buildDetailItem('Vendor', service.vendorName),
                    ),
                ...
                // ✅ Always show Change Package button (unless event within 24h)
                if (canModify) ...{
                  SizedBox(
                    width: double.infinity,
                    child: ElevatedButton.icon(
                      onPressed: () => _changePackage(event, service),
                      icon: const Icon(Icons.swap_horiz, size: 18),
                      label: Text(
                        isApproved ? 'Change Package' : 'Change Package (Pending)',
                        style: const TextStyle(fontSize: 13),
                      ),
                      style: ElevatedButton.styleFrom(
                        backgroundColor: isApproved ? Colors.orange : Colors.blue,
                  ...
                  // ✅ Remove button only for OPTIONAL services
                  if (!service.isRequired) ...{
                    const SizedBox(height: 8),
                    SizedBox(
                      width: double.infinity,
                      child: OutlinedButton.icon(
                        onPressed: () => _removePackage(event, service),
                        icon: const Icon(Icons.delete, size: 16),
                        label: const Text('Remove Package'),
  ...
  /// ============================================
  /// Change/Remove Methods
  /// ============================================
  void _changePackage(EventModel event, EventService service) {
    Navigator.push(
      context,
      MaterialPageRoute(
        builder: (context) => BrowsePackagesScreen(
          ...
          budgetData: {
            ...
            'remainingBudget': event.remainingBudget + service.packagePrice,
            'currency': 'EGP',
          },
          servicesData: {
            'selectedServices': [
              {
                ...
                'isReplacement': true,
                'currentPackageId': service.packageId, // ✅ الـ package الحالي
                'currentPackageName': service.packageName,
              },
            ],
          },
        ),
      ...
      // ✅ استقبل الـ result من BrowsePackagesScreen
      if (result != null && result is Map) {
        debugPrint('🔄 [_changePackage] Received result: $result');
        final newPackage = result['package'] as PackageModel?;
        final serviceId = result['serviceId'] as String?;
        
        if (newPackage != null && serviceId != null) {
          // ✅ استدعي replacePackage من الـ Cubit
          _replacePackageInFirestore(event, serviceId, newPackage);
        }
      }
    });
  }

  /// ✅ Replace Package in Firestore
  void _replacePackageInFirestore(EventModel event, String serviceId, PackageModel newPackage) {
    debugPrint('🔄 [_replacePackageInFirestore] Replacing package...');
    debugPrint('   Event: ${event.eventId}');
    debugPrint('   Service: $serviceId');
    debugPrint('   New Package: ${newPackage.packageName}');
    ...
    // ✅ استدعي replacePackage من الـ Cubit
    context.read<EventOwnerCubit>().replacePackage(
      eventId: event.eventId,
      serviceId: serviceId,
      newPackageId: newPackage.packageId,
      newPackageName: newPackage.packageName,
      newVendorId: newPackage.vendorId,
      newVendorName: newPackage.vendorName,
        ...
        content: const SingleChildScrollView(
          child: Column(
            ...
            children: [
              ...
              SizedBox(height: 8),
              Text('• Available anytime before event'),
              Text('• Select a different package from same service'),
              Text('• Old request will be cancelled'),
              Text('• New request requires vendor approval'),
              SizedBox(height: 16),
              Text('❌ Remove Package:',
                  style: TextStyle(fontWeight: FontWeight.bold)),
              SizedBox(height: 8),
              ...
              Text('🔒 Locked Period:',
                  style: TextStyle(fontWeight: FontWeight.bold)),
              SizedBox(height: 8),
              Text('• Changes locked within 24h of event'),
              Text('• Contact support for urgent changes'),
            ],
          ),
        ),
        actions: [
